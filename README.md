...
- **Dopo il fix precedente, mappa ancora vuota E luoghi utili che falliscono con "controlla la connessione", pur con connessione attiva**: entrambe le funzioni (tile della mappa da OpenFreeMap, luoghi utili da Overpass API) richiedono internet in tempo reale, e l'app non ha ancora una cache offline (pianificata, non ancora implementata — vedi "Mappe offline" nel piano di sviluppo). Un'icona di connessione attiva sul telefono non garantisce che il traffico dati funzioni davvero in quel punto: segnale debole/intermittente in zone rurali o montane (lo scenario tipico di questa app) può far fallire silenziosamente sia il download dei tile sia le chiamate a Overpass, pur mostrando "connesso". Per distinguere questo caso da un bug vero e proprio, la schermata "Luoghi utili" ora mostra anche il dettaglio tecnico dell'errore (tipo di eccezione e messaggio) invece del solo messaggio generico — utile per capire se si tratta di timeout, DNS, o altro, senza bisogno di Logcat.
