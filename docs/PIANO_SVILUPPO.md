...
1. **Tracciamento del percorso effettuato**: registrazione del cammino realmente fatto — sia quando si segue una traccia GPX importata, sia quando si parte semplicemente indicando un punto di partenza e uno di arrivo senza GPX. Al termine, salvare il percorso con tempo impiegato, distanza in km e, se ricavabile dal GPS/altimetro del telefono, dislivello. È il tracciamento "attività" standard delle app di questo tipo (Strava, Komoot, ecc.) — probabilmente richiede di introdurre Room per la persistenza locale (già pianificata in Fase 2 per la cache POI, si può accorpare).
   - **Pulsante di pausa**: interrompe il conteggio di tempo/distanza durante una sosta, senza fermare la registrazione dell'intero percorso.
   - **Avviso automatico "sei ancora in pausa?"**: se durante la pausa il GPS rileva che l'utente ha ripreso a camminare (spostamento continuo oltre una soglia), l'app avvisa con una notifica/banner per ricordare di premere "Riprendi" — evita che tempo e distanza vengano persi per essersi dimenticati di uscire dalla pausa.
...
4. **Scoperta cammini nelle vicinanze, con download GPX**: dalla posizione corrente, mostrare un elenco di percorsi che partono da lì o nei dintorni, con punto di arrivo, km, tempo previsto e livello di difficoltà, e possibilità di scaricare la traccia GPX corrispondente per usarla nell'app.
...
- **Dipendenza da infrastruttura Overpass API pubblica**: il mirror usato oggi (`overpass.kumi.systems`) potrebbe in futuro applicare gli stessi filtri anti-bot del server principale, o diventare instabile/sovraccarico — è successo al server principale nel 2026 (vedi problema HTTP 406 sopra). Da tenere d'occhio; se si ripresenta, prima cosa da controllare è il dettaglio tecnico dell'errore in "Luoghi utili" (già mostrato in UI dalla v1.6). Soluzione strutturale a lungo termine, se il problema si ripete: scaricare periodicamente un estratto `.osm.pbf` (es. da Geofabrik) e servire i luoghi utili da una copia propria dei dati invece che da un server Overpass pubblico condiviso.
