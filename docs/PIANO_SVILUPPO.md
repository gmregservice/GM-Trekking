...
2. **Cronologia percorsi**: schermata per rivedere l'elenco dei percorsi salvati dalla registrazione del punto 1 (data, distanza, tempo in movimento, dislivello per ciascuno). I dati sono già scritti su file da `ActivityStorage.kt` (`loadAll()` già pronta per leggerli) — manca solo la schermata che li mostra. Punto naturale d'ingresso: una nuova voce/icona nella barra in alto di `MainMapScreen.kt`, verso una nuova `ActivityHistoryScreen`.
