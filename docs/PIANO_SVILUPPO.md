...
6. **Luoghi utili: distanza e navigazione verso il punto**: nell'elenco "Luoghi utili", mostrare accanto al nome la distanza dalla posizione corrente (già calcolabile: `Poi` ha latitudine/longitudine, la posizione corrente è già disponibile dove viene mostrato l'elenco); selezionando un luogo, poter avviare la navigazione verso quel punto — probabilmente riusando `NavigationEngine`/`TrekMapView` già scritti per i tracciati GPX, ma con un "percorso" di un solo punto di arrivo invece di un tracciato importato. Annotato su richiesta esplicita (agosto 2026), non ancora iniziato.
